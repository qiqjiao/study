...
#include <ngx_core.h>
#include <ngx_http.h>
static char *ngx_http_hello(ngx_conf_t *cf, void *post, void *conf);
/* This variable is essential, it will cause error if we use 
   ngx_http_hello directly */
 ...
 */
static char *
ngx_http_hello(ngx_conf_t *cf, void *post, void *conf)
{
    ngx_http_core_loc_conf_t *clcf;
    ngx_http_hello_loc_conf_t *hlcf = conf;
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
    clcf->handler = ngx_http_hello_handler;
    if (ngx_strcmp(hlcf->name.data, "") == 0) {
